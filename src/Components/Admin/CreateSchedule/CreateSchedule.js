...
const CreateSchedule = ({ onClose }) => {
    ...
    const fetchDropdownData = async () => {
      try {
        const [driversResponse, routesResponse] = await Promise.all([
          axios.get("https://localhost:7032/api/Admin/getAllDriversByAdmin", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get("https://localhost:7032/api/Student/getAllRoutes", {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(
        "https://localhost:7032/api/Admin/CreateSchedule",
        payload,
        {
          headers: {
